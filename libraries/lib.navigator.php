...
class Nav extends Engine{
    ...
    private $filePath = WEB_PUBLIC.DS;
    ...
    public function nav_option($pg,$option=null,$target=null){
        $result = $this->filePath."dashboard/platform.php";
        $page = $this->decodeURL($pg);
        if($option!==null && $target==null){
            $pageoptions = $this->decodeURL($option);
            $pathurl = $this->filePath.$page.'/'.$pageoptions.'/platform.php';
            ...
            if(file_exists($this->filePath.$page.'/'.$pageoptions.'/'.$path)){
                $result= $path;
            }
        }
        return $result;
