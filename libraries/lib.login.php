<?php
		...
		private function signin(){
			...
			if($stmt){
				if($stmt->RecordCount() > 0){
					...
					$this->storeAuth($userid,USER_LOGIN_VAR,$infullname,$useraccesslevel,$branchcode,$companycode,$loginstatus,$accstatus,$logintime,$useremail,$userphoto,$userfirstname);
					$this->setLog("1");
					...
					$query = "INSERT INTO ".$this->prefix."eventlog (EVL_EVTCODE,EVL_USERID,EVL_FULLNAME,EVL_ACTIVITIES,EVL_IP,EVL_SESSION_ID,EVL_BROWSER,EVL_RAW) VALUES (".$this->connect->Param('a').",".$this->connect->Param('b').",".$this->connect->Param('c').",".$this->connect->Param('d').",".$this->connect->Param('e').",".$this->connect->Param('f').",".$this->connect->Param('g').",".$this->connect->Param('h').")";
	...
	public function storeAuth($userid,$login,$infullname,$useraccesslevel,$branchcode,$companycode,$loginstatus,$accstatus,$logintime,$useremail,$userphoto,$userfirstname){
		...
		$this->connect->Execute("UPDATE ".$this->prefix."users SET USR_LOGIN_STATUS = '1',USR_LASTLOGIN_DATE = ".$this->connect->Param('a').", USR_LOGIN_IP = ".$this->connect->Param('b')." WHERE USR_ID = ".$this->connect->Param('e'),array(date("Y-m-d H:i:s"),$this->remoteip,$userid));
	}//end
	...
	public function logout($msg="out"){
		...
		if($msg =="out"){
			...
			$this->connect->Execute("UPDATE ".$this->prefix."users SET USR_LOGIN_STATUS = '0',USR_LASTLOGIN_DATE = ".$this->connect->Param('a')." WHERE USR_ID = ".$this->connect->Param('e'),array(date("Y-m-d H:i:s"),$userid));
			$this->setLog("0");
		}
	...
	public function confirmAuth(){
		...
		if(md5($this->hashkey . $login .$this->remoteip.$this->sessionid.$this->useragent) != $hashkey){
			...
			//UPDATE SESSION
			$userid=$this->session->get("actorid");
			$this->connect->Execute("UPDATE ".$this->prefix."users SET USR_LASTLOGIN_DATE = ".$this->connect->Param('a')." WHERE USR_ID = ".$this->connect->Param('e'),array(date("Y-m-d H:i:s"),$userid));
		}
	...
	private function setLog($act){
		$userid=$this->session->get("actorid");
		$ufullname = $this->session->get('userfullname');
		$toinsetsession = $this->session->getSessionID();
		$raw = serialize($_SERVER);
		$query = "INSERT INTO ".$this->prefix."eventlog (EVL_EVTCODE,EVL_USERID,EVL_FULLNAME,EVL_ACTIVITIES,EVL_IP,EVL_SESSION_ID,EVL_BROWSER,EVL_RAW) VALUES (".$this->connect->Param('a').",".$this->connect->Param('b').",".$this->connect->Param('c').",".$this->connect->Param('d').",".$this->connect->Param('e').",".$this->connect->Param('f').",".$this->connect->Param('g').",".$this->connect->Param('h').")";
		if($act == "1"){
