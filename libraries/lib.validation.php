...
{    
    ...
    /**
     * @var array Holds all the real name or label names for the fields
     */
    private $_validationLabelName;
     ...
     */
    public function setValidation($arrayName, $validationRules,$labelName)
    {
        $this->_validationRules[$arrayName] = $validationRules;
        $this->_validationLabelName[$arrayName] = strtoupper($labelName);
    }
    ...
    /**
     * -------------------------------------------------------------
     * Bring out the error on validation in an array
     * @param void
     */
    ...
    {        
        ...
        { 
            $this->_validationErrors[$key] = $this->_validationLabelName[$key].' : This field is required';
        }
    }
    ...
    private function validEmail($key, $value, $ruleValue)
    {
        ...
        if(!preg_match("/^([a-z0-9\+_\-]+)(\.[a-z0-9\+_\-]+)*@([a-z0-9\-]+\.)+[a-z]{2,6}$/ix", $value) && $ruleValue == TRUE)
        {
            ...
            {
                $this->_validationErrors[$key] = $this->_validationLabelName[$key].' : Please fill in a valid email addresss';
            }
        }
    }
    ...
    private function validURL($key, $value, $ruleValue)
    {
        ...
        if(!preg_match('/^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$/', $value) && $ruleValue == TRUE)
        {
            ...
            if(!isset($this->_validationErrors[$key]) && !empty($value))
            {
                $this->_validationErrors[$key] = $this->_validationLabelName[$key].' : This is not a valid URL';
            }
        }
    }
    ...
    private function match($key, $value, $ruleValue)
    {
        ...
        {
            $this->_validationErrors[$key] = $this->_validationLabelName[$key] . ' does not match '. $this->_validationLabelName[$ruleValue];
        }
    }
    ...
    private function minLength($key, $value, $ruleValue)
    {
        ...
        if(strlen($value) < $ruleValue)
        {
            ...
            if(!empty($value))
            {
                $this->_validationErrors[$key] = $this->_validationLabelName[$key] . ' must be longer than '.$ruleValue.' characters';
            }
        }
    }
    ...
    private function maxLength($key, $value, $ruleValue)
    {
        ...
        if(strlen($value) > $ruleValue)
        {
            ...
            if(!empty($value))
            {
                $this->_validationErrors[$key] = $this->_validationLabelName[$key] . ' must be shorter than '.$ruleValue.' characters';
            }
        }
    }
    ...
    private function exactLength($key, $value, $ruleValue)
    {
        ...
        if(strlen($value) != $ruleValue)
        {
            ...
            if(!empty($value))
            {
                $this->_validationErrors[$key] = $this->_validationLabelName[$key] . ' must be '.$ruleValue.' characters long';
            }
        }
    }
    ...
    private function alpha($key, $value, $ruleValue)
    {
        ...
        if(!preg_match('/^([a-z])+$/i', $value))
        {
            ...
            if(!isset($this->_validationErrors[$key]) && !empty($value))
            {
                $this->_validationErrors[$key] = $this->_validationLabelName[$key] . ' can only contain letters';
            }
        }
    }
    ...
    private function alphaNumeric($key, $value, $ruleValue)
    {
        ...
        if(!preg_match('/^([a-z0-9])+$/i', $value))
        {
            ...
            if(!isset($this->_validationErrors[$key]) && !empty($value))
            {
                $this->_validationErrors[$key] = $this->_validationLabelName[$key] . ' can only contain letters and numbers';
            }
        }
    }
    ...
    private function numeric($key, $value, $ruleValue)
    {
        ...
        if(!preg_match('/^[\-+]?[0-9]*\.?[0-9]+$/', $value))
        {
            ...
            if(!isset($this->_validationErrors[$key]) && !empty($value))
            {
                $this->_validationErrors[$key] = $this->_validationLabelName[$key] . ' can only contain numbers';
            }
        }
    }
    ...
    private function lessThan($key, $value, $ruleValue)
    {
        ...
        if(preg_match('/^[\-+]?[0-9]*\.?[0-9]+$/', $value))
        {
            ...
            if($value >= $ruleValue)
            {
                ...
                if(!isset($this->_validationErrors[$key]) && !empty($value))
                {
                    $this->_validationErrors[$key] = $this->_validationLabelName[$key] . ' : Must be less than '.$ruleValue;
                }
            }
        }
        else
    ...
    private function greaterThan($key, $value, $ruleValue)
    {
        ...
        if(preg_match('/^[\-+]?[0-9]*\.?[0-9]+$/', $value))
        {
            ...
            if($value <= $ruleValue)
            {
                if(!isset($this->_validationErrors[$key]) && !empty($value))
                {            
                    $this->_validationErrors[$key] = $this->_validationLabelName[$key] . ' : Must be higher than '.$ruleValue;
                }
            }
        }
        else
