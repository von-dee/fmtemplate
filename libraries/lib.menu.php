...
class Menu extends Engine{
	...
	public function init_root_menu(){
		$sql = $this->sql;
		$stmt = $sql->Execute($sql->Prepare("SELECT RMN_CODE,RMN_NAME,RMN_ICON,RMN_SLUG FROM ".$this->prefix."root_menu WHERE RMN_STATUS='1' "));
		if($stmt && $stmt->RecordCount() >0){
			$root = $stmt->GetAll();
		}else{
			$root =[];
	...
	public function has_submenu($value,$array,$key){
		return in_array($value, array_column($array, $key));
	}
